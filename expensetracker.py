...
def login():
    if request.method == 'POST':
        ...
        cur.execute('SELECT user_id, password FROM expenses WHERE username = %s', (username,))
        user = cur.fetchone()
        if user:
            ...
            if stored_password == password:
                ...
                flash('Nope! I think you made a mistake... try again.', 'error')
        else:
            flash('Nope! I think you made a mistake... try again.', 'error')
...
def register():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute('SELECT username FROM expenses WHERE username = %s', (username,))
        existing_user = cur.fetchone()
        if existing_user:
            flash('Someone stole your name. Try a new one!', 'error')
        else:
            try:
                cur.execute('INSERT INTO expenses (user_id, username, password) VALUES (%s, %s, %s)', 
